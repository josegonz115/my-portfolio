...
export function DetailContact({ item }: Props) {
  ...
  const linkColor = cyber ? 'rgba(0,212,170,0.6)' : 'rgba(255,255,255,0.6)';
  const borderDim = cyber ? 'rgba(0,212,170,0.3)' : 'rgba(255,255,255,0.3)';
  const underlineColor = cyber ? 'rgba(0,212,170,0.2)' : 'rgba(255,255,255,0.2)';
  ...
  const icon =
    type === 'email' ? (
      ...
      <svg viewBox="0 0 24 24" className="w-6 h-6" style={{ color: textMid }} fill="currentColor">
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-8 py-16">
      <div className="p-4 rounded-full" style={{ border: `2px solid ${borderDim}` }}>
        {icon}
      </div>

      <div className="flex flex-col items-center gap-2">
        <h2 className="text-lg tracking-[0.2em] uppercase font-bold" style={{ fontFamily: FONT_HEADER, color: ACCENT }}>
          {item.label}
        </h2>
        <span className="text-[10px] tracking-[0.3em] uppercase" style={{ fontFamily: FONT_MONO, color: textFaint }}>
          {type === 'email' ? 'Contact' : 'Link'}
        </span>
      </div>
      ...
      <a
        ...
        target={type === 'email' ? undefined : '_blank'}
        rel={type === 'email' ? undefined : 'noopener noreferrer'}
        className="mt-2 flex items-center gap-2 px-6 py-3 text-[10px] tracking-widest uppercase"
        style={{
