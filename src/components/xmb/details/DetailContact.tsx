...
export function DetailContact({ item }: Props) {
  ...
  const icon =
    type === 'email' ? (
      ...
      <svg viewBox="0 0 24 24" className="w-10 h-10" style={{ color: textMid }} fill="currentColor">
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-8 py-16">
      <div className="p-6 rounded-full" style={{ border: `2px solid ${borderDim}` }}>
        {icon}
      </div>
      ...
      <a
        ...
        target={type === 'email' ? undefined : '_blank'}
        rel={type === 'email' ? undefined : 'noopener noreferrer'}
        className="mt-2 flex items-center gap-2 px-6 py-3 text-[14px] tracking-widest uppercase"
        style={{
